...
class MI2 extends events_1.EventEmitter {
    ...
    load(cwd, target, procArgs, separateConsole, autorun) {
    ...
    ssh(args, cwd, target, procArgs, separateConsole, attach, autorun) {
        return new Promise((resolve, reject) => {
            this.isSSH = true;
            this.sshReady = false;
    ...
    connectToQnx(cwd, executable, targetFIlePath, remotePort, autorun) {
        return new Promise((resolve, reject) => {
            let args = [];
            if (executable && !path.isAbsolute(executable))
                executable = path.join(cwd, executable);
