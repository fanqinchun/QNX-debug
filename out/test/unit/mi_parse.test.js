...
suite("MI Parse", () => {
    test("Very simple out of band record", () => {
        const parsed = (0, mi_parse_1.parseMI)(`*stopped`);
        assert.ok(parsed);
    ...
    test("Simple out of band record", () => {
        const parsed = (0, mi_parse_1.parseMI)(`4=thread-exited,id="3",group-id="i1"`);
        assert.ok(parsed);
    ...
    test("Console stream output with new line", () => {
        const parsed = (0, mi_parse_1.parseMI)(`~"[Thread 0x7fffe993a700 (LWP 11002) exited]\\n"`);
        assert.ok(parsed);
        assert.strictEqual(parsed.token, undefined);
        assert.strictEqual(parsed.outOfBandRecord.length, 1);
    ...
    test("Unicode", () => {
        let parsed = (0, mi_parse_1.parseMI)(`~"[Depuraci\\303\\263n de hilo usando libthread_db enabled]\\n"`);
        assert.ok(parsed);
        assert.strictEqual(parsed.token, undefined);
        assert.strictEqual(parsed.outOfBandRecord.length, 1);
        assert.strictEqual(parsed.outOfBandRecord[0].isStream, true);
        ...
        assert.strictEqual(parsed.resultRecords, undefined);
        parsed = (0, mi_parse_1.parseMI)(`~"4\\t  std::cout << \\"\\345\\245\\275\\345\\245\\275\\345\\255\\246\\344\\271\\240\\357\\274\\214\\345\\244\\251\\345\\244\\251\\345\\220\\221\\344\\270\\212\\" << std::endl;\\n"`);
        assert.ok(parsed);
        assert.strictEqual(parsed.token, undefined);
        assert.strictEqual(parsed.outOfBandRecord.length, 1);
        assert.strictEqual(parsed.outOfBandRecord[0].isStream, true);
    ...
    test("Empty line", () => {
        const parsed = (0, mi_parse_1.parseMI)(``);
        assert.ok(parsed);
        assert.strictEqual(parsed.token, undefined);
    ...
    test("'(gdb)' line", () => {
        const parsed = (0, mi_parse_1.parseMI)(`(gdb)`);
        assert.ok(parsed);
        assert.strictEqual(parsed.token, undefined);
        assert.strictEqual(parsed.outOfBandRecord.length, 0);
        assert.strictEqual(parsed.resultRecords, undefined);
    ...
    test("Simple result record", () => {
        const parsed = (0, mi_parse_1.parseMI)(`1^running`);
        assert.ok(parsed);
    ...
    test("Advanced out of band record (Breakpoint hit)", () => {
        const parsed = (0, mi_parse_1.parseMI)(`*stopped,reason="breakpoint-hit",disp="keep",bkptno="1",frame={addr="0x00000000004e807f",func="D main",args=[{name="args",value="..."}],file="source/app.d",fullname="/path/to/source/app.d",line="157"},thread-id="1",stopped-threads="all",core="0"`);
        assert.ok(parsed);
        assert.strictEqual(parsed.token, undefined);
        assert.strictEqual(parsed.outOfBandRecord.length, 1);
        assert.strictEqual(parsed.outOfBandRecord[0].isStream, false);
    ...
    test("Advanced result record", () => {
        const parsed = (0, mi_parse_1.parseMI)(`2^done,asm_insns=[src_and_asm_line={line="134",file="source/app.d",fullname="/path/to/source/app.d",line_asm_insn=[{address="0x00000000004e7da4",func-name="_Dmain",offset="0",inst="push   %rbp"},{address="0x00000000004e7da5",func-name="_Dmain",offset="1",inst="mov    %rsp,%rbp"}]}]`);
        assert.ok(parsed);
    ...
    test("valueof children", () => {
        ...
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[0], "@frame.level"), "0");
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[0], "@frame.addr"), "0x0000000000435f70");
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[0], "@frame.func"), "D main");
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[0], "@frame.file"), "source/app.d");
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[0], "@frame.fullname"), "/path/to/source/app.d");
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[0], "@frame.line"), "5");
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[1], "@frame.level"), "1");
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[1], "@frame.addr"), "0x00000000004372d3");
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[1], "@frame.func"), "rt.dmain2._d_run_main()");
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[1], "@frame.file"), undefined);
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[1], "@frame.fullname"), undefined);
        assert.strictEqual(mi_parse_1.MINode.valueOf(obj[1], "@frame.line"), undefined);
    });
    test("empty string values", () => {
        const parsed = (0, mi_parse_1.parseMI)(`15^done,register-names=["r0","pc","","xpsr","","control"]`);
    ...
    test("empty array values", () => {
        const parsed = (0, mi_parse_1.parseMI)(`15^done,foo={x=[],y="y"}`);
